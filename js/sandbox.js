...
function mountLabPanel() {
  ...
  p.innerHTML =
    ...
    '</label>' +
    '<label class="slot-panel-field">Meter <span data-meter-out>0 pulls &middot; 0.005%</span>' +
      '<input type="range" min="0" max="250" step="1" value="0" class="slot-range" data-act="meter">' +
    '</label>' +
  ...
  p.querySelector('[data-act="meter"]').addEventListener('input', e => {
    ...
    // From jackpotOdds() rather than a third copy of the ramp. The copy that
    // used to live here went stale the moment the ramp was retuned, and a lab
    // that reports the retired odds is worse than one that reports none.
    const pct = jackpotOdds(pulls) * 100;
    p.querySelector('[data-meter-out]').textContent =
      pulls + ' pulls \u00b7 ' + pct.toFixed(pct < 1 ? 3 : 1) + '%';
  });
